            ...
            <el-dropdown v-if="showMenu" trigger="click" @command="handleCommand">
                ...
                <template #dropdown>
                    <el-dropdown-menu>
                        ...
                        <el-dropdown-item v-if="status === 'SUCCESS' && ['FLINK_SQL', 'QUERY_JDBC', 'FLINK_JAR','PRQL'].includes(node.data.nodeConfigData.workType)" command="node_result">运行结果</el-dropdown-item>
                        <el-dropdown-item v-if="['FLINK_SQL', 'BASH', 'PYTHON', 'DATA_SYNC_JDBC'].includes(node.data.nodeConfigData.workType)" command="node_TaskManagerLog">运行日志</el-dropdown-item>
