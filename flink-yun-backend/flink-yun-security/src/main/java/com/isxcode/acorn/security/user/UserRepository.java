...
public interface UserRepository extends JpaRepository<UserEntity, String> {
     ...
     * 分页搜索查询所有用户.
     */
    @Query("SELECT U FROM UserEntity U " + "WHERE U.roleCode != 'ROLE_SYS_ADMIN' and " + "(U.username LIKE %:keyword% " + "OR U.account LIKE %:keyword% " + "OR U.email LIKE %:keyword% "
    ...
    @Query("SELECT U FROM UserEntity U " + "WHERE U.roleCode != 'ROLE_SYS_ADMIN' " + "AND U.status = 'ENABLE' " + "AND (U.username LIKE %:keyword%" + " OR U.account LIKE %:keyword% "
            + "OR U.email LIKE %:keyword% " + "OR U.phone LIKE %:keyword% " + "OR U.remark LIKE %:keyword%) order by U.createDateTime desc ")
