...
import com.isxcode.acorn.modules.work.mapper.WorkMapper;
...
import com.isxcode.acorn.modules.work.service.WorkConfigService;
...
public class WorkBizService {
    ...
    public GetWorkRes addWork(AddWorkReq addWorkReq) {
        ...
        // flinkSql，数据同步，bash，python，必须配置集群
        if (WorkType.FLINK_SQL.equals(addWorkReq.getWorkType())
        ...
        // 如果是flinkSql,jdbcQuerySql,jdbcExecuteSql,bash,python作业，需要初始化脚本内容，方便客户使用
        if (WorkType.FLINK_SQL.equals(addWorkReq.getWorkType())
        ...
        if (WorkType.FLINK_CONTAINER_SQL.equals(addWorkReq.getWorkType())) {
    ...
    public GetStatusRes getStatus(GetStatusReq getStatusReq) {
        ...
        if (Strings.isEmpty(workInstanceEntity.getFlinkStarRes())) {
            throw new IsxAppException("请等待作业提交完毕");
        }

        return JSON.parseObject(workInstanceEntity.getFlinkStarRes(), GetStatusRes.class);
    }
    ...
    public GetWorkLogRes getWorkLog(GetTaskManagerLogReq getTaskManagerLogReq) {

        Optional<WorkInstanceEntity> workInstanceEntityOptional =
            workInstanceRepository.findById(getTaskManagerLogReq.getInstanceId());
        if (!workInstanceEntityOptional.isPresent()) {
    ...
    public GetWorkRes getWork(GetWorkReq getWorkReq) {
        ...
        if (!Strings.isEmpty(workConfig.getClusterConfig())) {
            ...
            getWorkRes.getClusterConfig()
                .setFlinkConfigJson(JSON.toJSONString(getWorkRes.getClusterConfig().getFlinkConfig()));
        }
