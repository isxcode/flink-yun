...
public class FileController {
    ...
    public void uploadFile(@RequestParam("file") @Schema(title = "文件") MultipartFile file, @RequestParam("type") @Schema(title = "文件类型") @Pattern(regexp = "^(JOB|FUNC|LIB)$", message = "只能是JOB/FUNC/LIB其中一个") String type, @RequestParam(value = "remark", required = false) @Schema(title = "备注") String remark) {
    ...
    public void updateFile(@RequestParam("fileId") @Schema(title = "资源文件id") String fileId, @RequestParam(value = "file", required = false) @Schema(title = "文件") MultipartFile file, @RequestParam(value = "remark", required = false) @Schema(title = "备注") String remark) {
