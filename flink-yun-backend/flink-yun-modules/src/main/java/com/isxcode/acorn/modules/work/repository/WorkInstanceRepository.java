...
public interface WorkInstanceRepository extends JpaRepository<WorkInstanceEntity, String> {
    ...
    @Query(value = "" + "select S.id as id,\n" + "       SW.name  as workName,\n" + "       SWF.name as workflowName,\n" + "       S.instanceType as instanceType,\n"
            ...
            + "WHERE S.tenantId=:tenantId and " + "(S.id LIKE %:keyword% " + "OR SW.name LIKE %:keyword% " + "OR SWF.name LIKE %:keyword% ) " + "order by S.lastModifiedDateTime desc ")
    ...
    @Query("SELECT W FROM WorkInstanceEntity W " + "WHERE W.workId IN (:workIds) " + "AND W.workflowInstanceId = :workflowInstanceId ")
