...
if ! docker image inspect flink:1.18.1-scala_2.12 &>/dev/null; then
  json_output="{ \
            \"status\": \"INSTALL_ERROR\", \
            \"log\": \"没有flink:1.18.1-scala_2.12镜像，需要执行拉取镜像命令，docker pull flink:1.18.1-scala_2.12 或者 docker pull registry.cn-shanghai.aliyuncs.com/isxcode/flink:1.18.1-scala-2.12-amd64 && docker tag registry.cn-shanghai.aliyuncs.com/isxcode/flink:1.18.1-scala-2.12-amd64 flink:1.18.1-scala_2.12\" \
          }"
  echo $json_output
  rm ${BASE_PATH}/agent-kubernetes.sh
  exit 0
