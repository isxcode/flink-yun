...
public class OpenApiConfig {
	...
	public OpenAPI customOpenApi() {
		...
		SecurityScheme basicAuthScheme = new SecurityScheme().type(SecurityScheme.Type.APIKEY)
				.in(SecurityScheme.In.HEADER).description("输入用户token").scheme("basic")
				.name(SecurityConstants.HEADER_AUTHORIZATION);

		SecurityScheme basicTenantScheme = new SecurityScheme().type(SecurityScheme.Type.APIKEY)
				.in(SecurityScheme.In.HEADER).description("输入租户tenant").scheme("tenant")
