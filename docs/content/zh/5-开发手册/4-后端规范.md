...
##### flink-yun-backend/flink-yun-api
> 所有的java基础对象：
...
##### flink-yun-backend/flink-yun-modules
> 所有模块逻辑代码：
...
##### flink-yun-vip/flink-yun-backend
> 类似flink-yun-modules结构，该代码为闭源代码

### 定义模块
每个模块需要一个通俗易懂的名字  
编辑`com.isxcode.acorn.api.main.constants.ModuleCode`  
...
例如：
> flink-yun-main/src/main/resouces/db/migration/h2/V88__ALL.sql    
> flink-yun-main/src/main/resouces/db/migration/mysql/V88__ALL.sql  

注意：部分mysql的语法，在h2中无法运行，类似创建索引的sql，一定要本地试一下。  
建议：数据库中减少null值的使用，最好每个字段都有默认值   
表名规范：FY_XXX  (SY为flink_yun的缩写) XXX结尾不需要加S、表名全部大写、用下划线分割  
系统字段包括：id、create_by、create_date_time、last_modified_by、last_modified_date_time、version_number、deleted、tenant_id  
建表模版：

```sql
create table if not exists FY_DEMO_ISPONG
(
...
`com.isxcode.acorn.modules.<module_code>.entity.<module_code>Entity`   
...
@Data
@Entity
@SQLDelete(sql = "UPDATE FY_DEMO_ISPONG SET deleted = 1 WHERE id = ? and version_number = ?")
@Where(clause = "deleted = 0 ${TENANT_FILTER} ")
@Table(name = "FY_DEMO_ISPONG")
...
public class DemoIspongEntity {
  ...
  @GenericGenerator(
      ...
      strategy = "com.isxcode.acorn.config.GeneratedValueConfig")
  private String id;
...
基于Jpa对数据库持久化操作   
`com.isxcode.acorn.modules.<module_code>.repository.<module_code>Repository`   
函数命名规范：  
- findByXXX :  查询单个对象 
...
`com.isxcode.acorn.modules.<module_code>.entity.<custom>Do`   
...
@Query(value =
  "select "
    + " new com.isxcode.acorn.modules.demo_ispong.entity.CustomUserDo("
    + "   D.username,"
    + "   D.age,"
    + "   U.account) "
...
`com.isxcode.acorn.modules.<module_code>.controller.<module_code>Controller`    
所有接口一律使用Post   
接口命名规范: 
- 添加对象接口 :  addXXX 
- 更新对象接口 :  updateXXX 
...
public class ClusterController {
  ...
  public void importWorkflow(
        ...
        @Schema(description = "作业流唯一id", example = "fy_ba1f12b5c8154f999a02a5be2373a438") @RequestParam(required = false)
            String workflowId) {
...
`com.isxcode.acorn.modules.<module_code>.service.<module_code>Service`   
`com.isxcode.acorn.modules.<module_code>.service.biz.<module_code>BizService`   
...
`com.isxcode.acorn.modules.<module_code>.mapper.<module_code>Mapper`  
...
Spring 配置类   
`com.isxcode.acorn.api.main.properties.flinkYunProperties` 
...
import com.isxcode.acorn.api.datasource.pojos.res.GetConnectLogRes;
import com.isxcode.acorn.api.datasource.pojos.res.PageDatasourceRes;
import com.isxcode.acorn.api.datasource.pojos.res.TestConnectRes;
