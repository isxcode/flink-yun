...
给服务器3、服务器4、服务器5安装至流云服务，参考文档：[至流云离线部署](/docs/zh/1/2)
...
spring:
  ...
  datasource:
    ...
    url: jdbc:h2:file:/data/zhiliuyun/h2/data;AUTO_SERVER=TRUE
    username: root
    password: root1234

isx-app:
  resources-path: /data/zhiliuyun
...
```bash
mkdir -p /data/zhiliuyun
cp -r zhiliuyun/resources/jdbc /data/zhiliuyun/
```
...
server {
    location / {
        ...
        proxy_pass http://zhiliuyun_servers/;
    }
}
```
