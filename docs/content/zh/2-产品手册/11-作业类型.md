...
- **SparkSql查询作业**: 选择计算集群，执行flinksql返回`提交日志`、`运行结果`、`运行日志` 
- **SparkSql容器作业**: 选择计算容器，执行flinksql返回`提交日志`、`运行结果`  
...
> 支持高级配置SparkConfig，配置参考链接: https://flink.apache.org/docs/3.4.1/configuration.html 
...
> 运行成功后，点击`数据返回`，查看flinkSql执行的返回数据
...
> 字段支持转换功能，支持flink官方的函数，参考链接: https://flink.apache.org/docs/latest/sql-ref-functions.html
