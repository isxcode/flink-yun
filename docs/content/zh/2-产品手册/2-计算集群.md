...
?> 计算集群： 负责支持FlinkSql作业运行的环境。支持k8s,yarn,standalone计算集群
...
用户需要先点击检测按钮，然后点击安装按钮，即可对该节点做监听。当状态为可用时，即可用于flink作业的运行。
...
```config
flink.kubernetes.driver.podTemplateFile    /home/ispong/pod-init.yaml
flink.kubernetes.executor.podTemplateFile    /home/ispong/pod-init.yaml
```