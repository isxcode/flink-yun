...
public class KubernetesAcorn implements AcornRun {
    ...
    public SubmitJobRes submitJob(SubmitJobReq submitJobReq) {
        ...
        try (KubernetesClusterDescriptor clusterDescriptor = kubernetesClusterClientFactory.createClusterDescriptor(flinkConfig)) {
            ClusterClientProvider<String> clusterClientProvider = clusterDescriptor.deploySessionCluster(clusterSpecification);
    ...
    public static void main(String[] args) {
        String flinkSql = "CREATE TABLE from_table(\n" + "    s_name STRING,\n" + "    s_suppkey INT\n" + ") WITH (\n"
            ...
            + "    'url'='jdbc:mysql://isxcode:40004/isxcode_db',\n" + "    'table-name'='users2',\n"
            + "    'driver'='com.mysql.cj.jdbc.Driver',\n" + "    'username'='root',\n"
            + "    'password'='Zhiqingyun@isxcode');" + "insert into to_table select s_name, s_suppkey from from_table";
